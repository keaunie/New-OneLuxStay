...
const normalizeMiamiQuoteBreakdown = (incomingBreakdown) => {
...
const getQuotePricing = (quoteData, listing, nights, guestsCount = 1) => {
  ...
  const buildPlanPricing = (plan, idx) => {
    ...
    const normalizedBreakdown = normalizeMiamiQuoteBreakdown(
      {
        accommodation,
        discountAmount,
        discountRate,
...
const [checkoutPromoCode, setCheckoutPromoCode] = useState("");
  ...
  const buildMapPopupContent = useCallback((popupListings = [], activeIndex = 0, popupKey = "") => {
    ...
    const parentListingId = listingGroupKey
      ? getListingId(
          (Array.isArray(miamiBeachParentListings) ? miamiBeachParentListings : []).find(
    ...
    `;
  }, [cardQuoteRates, miamiBeachParentListings, sectionCheckIn, sectionCheckOut, showMonthlyTotal]);
  ...
  const fetchCalendarMinNightsFromMulti = async (listingId, targetDate) => {
    ...
    try {
      ...
      const listingPool = baseListing
        ? getRelatedListingsByParentId(baseListing, miamiBeachListings)
        : [];
  ...
  const fetchApaleoCalendarMonth = async (listing, targetDate, { force = false } = {}) => {
    const listingId = getCalendarListingId(listing, miamiBeachListings);
    if (!listingId) return;
    const monthStart = new Date(targetDate.getFullYear(), targetDate.getMonth(), 1);
...
const applyCheckoutPromoCode = () => {
  ...
  const similarListingsForActiveRoute = useMemo(() => {
    ...
    const parentListings = miamiBeachParentListings.length
      ? miamiBeachParentListings
      : miamiBeachListings;
    ...
    return [...relatedParents, ...parentListings]
      ...
      .slice(0, 8);
  }, [isListingRoute, activeListing, miamiBeachParentListings, miamiBeachListings]);
    ...
    <div className="la-listing-shell">
      ...
      {(() => {
        ...
        const selectedPlan =
          ...
          quote?.pricing ||
          null;
        const breakdown = normalizeMiamiQuoteBreakdown(
      ...
      {isListingRoute ? (
        <div className="ols-listing-fullbleed ols-listing-fullbleed--neighborhood">
          <NeighborhoodHighlightsSection cityKey="miami" />
        </div>
      ) : null}
      {isListingRoute ? (
