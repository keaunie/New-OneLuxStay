...
const getQuotePricing = (quoteData, listing, nights, guestsCount = 1, promoConfig) => {
  ...
  const refundablePlanEntries = plansRaw
    .map((plan, idx) => ({ plan, idx, normalized: normalizeRatePlan(plan?.ratePlan || {}) }))
    .filter((entry) => !entry.normalized.isNonRefundable);
  const promoBaseEntry = refundablePlanEntries.find((entry) =>
    /signature|standard|refundable/i.test(entry.normalized.label)
  ) || refundablePlanEntries[0] || null;
  const plans = promoBaseEntry
    ? PROMO_TIERS.map((tier) => buildPlanPricing(promoBaseEntry.plan, promoBaseEntry.idx, tier)).filter(Boolean)
    : [];
...
const SECTION_STORIES = {
  ...
  "dubai-palm": {
    ...
    copy:
      "Palm stays bring resort ease: private beach access, spa mornings, and golden hour by the water. It's the quiet, comfortable side of Dubai.",
...
const CITY_TOUR_SLIDES = {
  ...
  "Palm Jumeirah": [
    ...
    },
    {
      ...
      subtitle: "quiet surroundings and curated dining.",
      copy:
...
export default function DubaiLandingPage() {
  const promoConfig = usePromoConfig("dubai");
  const {
...
const [checkoutPromoCode, setCheckoutPromoCode] = useState("");
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        const basesToTry = [apiBase].filter(
        ...
        requests.forEach((entry) => {
          ...
          const selectedQuotePlan =
            quotePlansRaw.find((plan, idx) => {
              const ratePlanMeta = plan?.ratePlan || {};
              const planId = ratePlanMeta?._id || plan?._id || `plan-${idx}`;
              return String(planId) === String(selectedPlan?.providerPlanId || "");
  ...
  const computedStayRestrictionMessage =
    sectionCheckIn && sectionCheckOut && sectionStayNights === 0
      ...
      : sectionStayNights > 0 && sectionStayNights < requiredMinNightsForSelection
        ? `This residence requires at least ${requiredMinNightsForSelection} nights. Your stay is ${sectionStayNights} nights, so add ${requiredMinNightsForSelection - sectionStayNights} more night${requiredMinNightsForSelection - sectionStayNights === 1 ? "" : "s"} to continue.`
        : "";
  ...
  const fetchCalendarMonth = async (
    ...
    try {
      ...
      const normalizedDays = Array.isArray(data?.days)
        ? data.days.map((day) => normalizeCalendarDayForUi(day, null)).filter(Boolean)
        : [];
  ...
  const fetchCalendarMinNightsFromMulti = async (listingId, targetDate, { force = false } = {}) => {
    const key = `${listingId}-${monthKey(targetDate)}`;
  ...
  const handleListingCalendarOpen = (open) => {
    ...
    if (isApaleoListing(activeListing)) {
      ...
      const apaleoBaseDate = parseDateValue(sectionCheckIn) || new Date();
      apaleoBaseDate.setDate(1);
      apaleoBaseDate.setHours(0, 0, 0, 0);
      setCalendarStartDate(apaleoBaseDate);
      setCalendarMonthIndex(0);
      fetchApaleoCalendarMonth(activeListing, apaleoBaseDate, { force: true });
      return;
    }
    const baseDate = parseDateValue(sectionCheckIn) || new Date();
    baseDate.setDate(1);
    ...
    fetchCalendarMinNightsFromMulti(listingId, baseDate, { force: true });
  };
  ...
  useEffect(() => {
    ...
    setSelectedRatePlans((prev) => {
      ...
      Object.entries(sectionQuotes).forEach(([listingId, quote]) => {
        ...
        const selectedPlan = plans.find((plan) => plan.id === next[listingId]);
        if (!selectedPlan) {
          next[listingId] = defaultId;
          changed = true;
        }
  ...
  useEffect(() => {
    ...
    // A quote belongs to one exact date/guest selection. Remove it immediately
    // when that selection changes so a previous total is never shown as current.
    setSectionAvailability([]);
    setSectionAvailabilityMap({});
    setSectionAvailabilityActive(false);
    setSectionQuotes({});
    ...
    if (
      !sectionCheckIn ||
      !sectionCheckOut ||
      (!activeListing && !activeSection?.listings?.length) ||
      isStayTooShort
    ) {
      activeAvailabilityControllerRef.current?.abort();
      return undefined;
    ...
    // fetchAvailabilityListings intentionally uses the state snapshot represented
    // by these dependencies; including its render-local identity would re-fetch forever.
  ...
  const confirmGuestCheckout = () => {
    ...
    setCheckoutGuestError("");
    const consentText =
      "By signing and continuing to payment, you agree to the Terms and Conditions and authorize OneLuxStay to charge the total amount shown for your reservation. A receipt and consent proof PDF will be emailed to you";
    const payload = {
      ...pendingCheckout,
      amount: numericAmount,
      guest: checkoutGuest,
    ...
    <div className="la-listing-shell">
      ...
      <section className="la-listing-hero la-listing-hero--mobile-top-logo">
        ...
        <div className="la-listing-hero__intro">
          <div>
            ...
            <h3>{activeListing?.title || formatListingLocationLabel(activeListing, "Dubai")}</h3>
      ...
      {(() => {
        ...
        const selectedPlan =
          ...
          quote?.pricing ||
          null;
        const selectedPromoShortfall =
          selectedPlan?.promoKey &&
          selectedPlan.promoKey !== "none" &&
          cityDateNightCount > 0 &&
          cityDateNightCount < selectedPlan.requiredNights
            ...
            <div className="la-unit-modal__sidebar">
              <div className="la-unit-modal__card la-unit-modal__booking-panel" id="la-rooms" aria-label="Availability check">
                ...
                <div className="la-unit-modal__availability-details">
                  {isStayTooShort ? (
                    <div className="la-unit-modal__promo-warning" role="alert">
                      <strong>Insufficient nights</strong>
                      <span>{stayTooShortMessage}</span>
                    </div>
                  ) : availability === false ? (
                ...
                {selectedPromoShortfall > 0 && (
                  <div className="la-unit-modal__promo-warning" role="status">
                    ...
                    <span>
                      This promotion requires {selectedPlan.requiredNights} nights. Your stay is {cityDateNightCount} nights, so add {selectedPromoShortfall} more night{selectedPromoShortfall === 1 ? "" : "s"} to receive the {Math.round(Number(promoConfig?.[selectedPlan.promoKey]) || 0)}% discount.
                    </span>
                  </div>
                )}
                {!isStayTooShort && availability !== false && sectionAvailabilityActive && planOptions.length > 0 && (
                  <div className="la-unit-modal__rate-plan">
                    ...
                    <select
                      ...
                      disabled={sectionAvailabilityLoading || promoApplyingId === listingId}
      ...
      {isListingRoute ? (
        <div className="ols-listing-fullbleed ols-listing-fullbleed--neighborhood">
          <NeighborhoodHighlightsSection cityKey="dubai" />
        </div>
      ) : null}
      {isListingRoute ? (
