...
const ScrollStack = ({
  ...
  const cardOffsetsRef = useRef(new Map());
  ...
  const updateCardTransforms = useCallback(() => {
    ...
    cardsRef.current.forEach((card, i) => {
      ...
      const cardTop = cardOffsetsRef.current.get(card) ?? getElementOffset(card);
  ...
  const innerClasses = `scroll-stack-inner pt-[12vh] px-6 md:px-10 pb-[45rem] min-h-screen ${innerClassName}`.trim();
