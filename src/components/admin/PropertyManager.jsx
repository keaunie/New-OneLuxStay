...
export default function PropertyManager({ apiBase, session, standalone = false }) {
  ...
  const migrateNextGuestyImage = async () => {
    const pending = images.find((image) => image.migration_status === "pending");
    if (!pending) { setNotice("There are no pending Guesty images for this property."); return; }
    setMigrating(true); setError(""); setNotice("Copying one Guesty image to R2…");
    setImages((current) => current.map((image) => image.id === pending.id ? { ...image, migration_status: "copying", migration_error: null } : image));
    try {
      const result = await service.migrateNextGuestyImage(selectedId);
      if (result.verified) setNotice("One Guesty image was copied to R2 and verified.");
      else if (result.failed) setError(result.image?.migration_error || "The Guesty image migration failed.");
      else setNotice("There are no pending Guesty images for this property.");
      await loadDetail(selectedId);
    } catch (err) {
      setError(err.message); await loadDetail(selectedId);
