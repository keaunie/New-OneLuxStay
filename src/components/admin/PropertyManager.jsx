...
const emptyAccessRoom = () => ({ room_label: "", wifi_network: "", wifi_password: "", door_lock_type: "", door_code: "", notes: "", deposit_amount: "", deposit_currency: "", parking_instructions: "", _persisted: false });
const accessForm = (secrets = []) => {
  ...
  return rows.map((row) => ({
    ...
    deposit_amount: text(row.deposit_amount), deposit_currency: text(row.deposit_currency),
    parking_instructions: text(row.parking_instructions), _persisted: true,
  }));
};
...
const propertyForm = (property = {}) => Object.fromEntries([
  "id", "property_code", "unit_number", "guesty_listing_id", "guesty_listing_type", "parent_property_id", "name", "slug", "property_type",
  "room_type", "website_status", "status", "content_sync_mode", "source_system", "address", "city", "country", "latitude",
  "longitude", "timezone", "accommodates", "bedrooms", "bathrooms", "beds", "bed_type", "min_nights", "max_nights", "size_sqm", "parking_details",
...
export default function PropertyManager({ apiBase, session, standalone = false }) {
  ...
  const [access, setAccess] = useState(null); const [accessLoaded, setAccessLoaded] = useState(false); const [accessLoading, setAccessLoading] = useState(false);
  ...
  const loadDetail = useCallback(async (id) => {
    ...
    try {
      ...
      setAccess(null); setAccessLoaded(false);
    } catch (err) { setError(err.message); setProperty(null); } finally { setDetailLoading(false); }
  }, [service]);

  const loadAccessSecrets = useCallback(async () => {
    if (!selectedId || accessLoaded || accessLoading) return;
    setAccessLoading(true); setError("");
    try { setAccess(accessForm(await service.getPropertyAccessSecrets(selectedId))); setAccessLoaded(true); }
    catch (err) { setError(err.message); } finally { setAccessLoading(false); }
  ...
  const toggleMigrationSelection = (imageId) => setSelectedMigrationIds((current) => current.includes(imageId) ? current.filter((id) => id !== imageId) : [...current, imageId]);
  const migrateSelectedGuestyImages = async () => {
    const selected = images.filter((image) => selectedMigrationIds.includes(image.id) && ["pending", "failed"].includes(image.migration_status));
    if (!selected.length) { setNotice("Select at least one pending or failed Guesty image."); return; }
    setMigrating(true); setError(""); setNotice(`Copying ${selected.length} selected image${selected.length === 1 ? "" : "s"} to R2…`);
    setImages((current) => current.map((image) => selectedMigrationIds.includes(image.id) ? { ...image, migration_status: "copying", migration_error: null } : image));
    let verified = 0; const failures = [];
    for (const image of selected) {
      try {
        const result = await service.migrateGuestyImage(selectedId, image.id);
        if (result.verified) verified += 1;
        else failures.push(result.image?.migration_error || `Image ${image.id} failed.`);
    ...
    setNotice(`${verified} of ${selected.length} selected image${selected.length === 1 ? "" : "s"} copied to R2.`);
    await loadDetail(selectedId); setMigrating(false);
  };
  ...
  return <div className={`property-manager ${standalone ? "is-standalone" : ""}`}>
    <aside className="pm-sidebar">
      ...
      <div className="pm-list">{loading ? <p>Loading properties…</p> : rows.map((item) => {
        ...
        return <button type="button" key={item.id} className={item.id === selectedId ? "is-active" : ""} onClick={() => choose(item.id)}>{imageUrl(primary) ? <img src={imageUrl(primary)} alt="" loading="lazy" /> : <span className="pm-placeholder">OLS</span>}<span><strong>{item.property_code || item.name || "Untitled"}</strong>{item.unit_number && <small>Unit: {item.unit_number}</small>}<small>{first(item.property_descriptions).title || item.name}</small><small>{[item.city, item.country].filter(Boolean).join(", ") || "Location not set"}</small><em>{item.website_status} · {item.status}</em></span></button>;
      })}</div>
    ...
    <main className="pm-editor">
      ...
      {property && form && !detailLoading && <>
        ...
        <section className="pm-panel">
          ...
          {tab === "Beds & capacity" && <>{renderFields([{ key: "accommodates", label: "Maximum guests", type: "number" }, { key: "bedrooms", label: "Bedrooms", type: "number" }, { key: "bathrooms", label: "Bathrooms", type: "number" }, { key: "beds", label: "Total beds", type: "number" }, { key: "bed_type", label: "General bed type" }, { key: "min_nights", label: "Minimum nights", type: "number" }, { key: "max_nights", label: "Maximum nights", type: "number" }, { key: "size_sqm", label: "Property size (m²)", type: "number" }])}<div className="pm-checks">{[["has_balcony", "Balcony"], ["has_parking", "Parking"], ["has_wifi", "Wi-Fi"]].map(([key, label]) => <label key={key}><input type="checkbox" checked={form[key]} onChange={(e) => mutateForm(key, e.target.checked)} />{label}</label>)}</div><p className="pm-help">"Parking" here is the guest-facing yes/no amenity flag — it's what the public chatbot reads. The exact space number/level still goes in the Access tab's "Parking instructions" field, which stays admin-only.</p><div className="pm-fields"><label className="is-wide">Parking details (guest-facing)<textarea rows="2" value={form.parking_details} placeholder='e.g. "Paid garage on-site, $25/night, 1-minute walk from the unit" — the chatbot may quote this verbatim, so never put an exact space or level number here' onChange={(e) => mutateForm("parking_details", e.target.value)} /></label></div><h3>Bed arrangements</h3>{beds.map((bed, index) => <div className="pm-bed" key={bed.id || index}><input placeholder="Room name" value={bed.room_name || ""} onChange={(e) => { const next = [...beds]; next[index] = { ...bed, room_name: e.target.value }; setBeds(next); setDirty(true); }} /><input placeholder="Bed type" value={bed.bed_type || ""} onChange={(e) => { const next = [...beds]; next[index] = { ...bed, bed_type: e.target.value }; setBeds(next); setDirty(true); }} /><input type="number" min="1" value={bed.quantity ?? 1} onChange={(e) => { const next = [...beds]; next[index] = { ...bed, quantity: e.target.value }; setBeds(next); setDirty(true); }} /><input value={bed.source_text || ""} readOnly title="Original source text" /><button onClick={() => { setBeds(beds.filter((_, i) => i !== index)); setDirty(true); }}>Remove</button></div>)}<div className="pm-actions"><button onClick={() => { setBeds([...beds, { room_name: "", bed_type: "", quantity: 1, source_text: "" }]); setDirty(true); }}>Add bed row</button><button onClick={saveBeds} disabled={saving}>Save beds</button><button onClick={saveProperty} disabled={saving}>Save capacity</button></div></>}
            ...
            {!accessLoading && access && <>
              ...
              {access.map((room, index) => <div className="pm-access-room" key={room._persisted ? room.room_label : `new-${index}`}>
                ...
                <div className="pm-fields">
                  ...
                  <label className="is-wide">Parking instructions<textarea rows="2" value={room.parking_instructions} placeholder="Exact space/level, e.g. &quot;Level 2 parking garage, space #322&quot; — admin-only, never shown to guests" onChange={(e) => mutateAccessRoom(index, "parking_instructions", e.target.value)} /></label>
