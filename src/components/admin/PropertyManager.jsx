  ...
  return <div className={`property-manager ${standalone ? "is-standalone" : ""}`}>
    <aside className="pm-sidebar">
      ...
      <div className="pm-list">{loading ? <p>Loading properties…</p> : rows.map((item) => {
        ...
        return <button type="button" key={item.id} className={item.id === selectedId ? "is-active" : ""} onClick={() => choose(item.id)}>{imageUrl(primary) ? <img src={imageUrl(primary)} alt="" loading="lazy" /> : <span className="pm-placeholder">OLS</span>}<span><strong>{item.property_code || item.name || "Untitled"}</strong>{item.guesty_nickname && <small>Guesty unit: {item.guesty_nickname}</small>}<small>{first(item.property_descriptions).title || item.name}</small><small>{[item.city, item.country].filter(Boolean).join(", ") || "Location not set"}</small><em>{item.website_status} · {item.status}</em></span></button>;
      })}</div>
