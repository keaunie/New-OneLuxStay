...
export default function PropertyManager({ apiBase, session, standalone = false }) {
  ...
  const toggleMigrationSelection = (imageId) => setSelectedMigrationIds((current) => current.includes(imageId) ? current.filter((id) => id !== imageId) : current.length >= 10 ? current : [...current, imageId]);
