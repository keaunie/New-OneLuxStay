...
const DEFAULT_GUESTS = Math.max(1, Math.min(99, Number(process.env.GOOGLE_ARI_DEFAULT_GUESTS || 2) || 2));
...
const normalizeCurrency = (value) => normalizeString(value).toUpperCase() || "USD";
...
const chunkRates = (rows = []) => {
  const chunks = [];
  let current = null;

  for (const row of rows) {
    const date = toIsoDate(row?.date);
    const nightlyRate = toMoney(row?.nightly_rate);
    const currency = normalizeCurrency(row?.currency);
    const available = parseBoolean(row?.is_available, true);

    if (!date || nightlyRate == null) continue;
    if (!available) continue;

    if (
      current &&
      current.currency === currency &&
      current.nightlyRate === nightlyRate &&
    ...
    current = { start: date, end: date, nightlyRate, currency };
    chunks.push(current);
  }
...
const fetchNightlyRateRows = async ({ table, hotelId, start, end }) => {
  const baseQuery = {
    select: "date,nightly_rate,currency,is_available",
    date: `gte.${start}`,
    order: "date.asc",
    limit: Math.max(14, MAX_DAYS + 7),
  };

  // Try property_id first (properties-mode tables), then listing_id.
  try {
...
const readRatesWithFallback = async ({ hotelId, start, end }) => {
  const candidates = [SUPABASE_AVAILABILITY_TABLE, ...FALLBACK_AVAILABILITY_TABLES];
  let lastError = null;

  for (const table of candidates) {
    try {
      const rows = await fetchNightlyRateRows({ table, hotelId, start, end });
      return { table, rows };
    } catch (error) {
      const msg = String(error?.message || "");
      lastError = error;
...
export async function handler(event = {}) {
  ...
  try {
    ...
    const guests = Math.max(1, Math.min(99, parsePositiveInt(query.guests, DEFAULT_GUESTS)));
    const notifType = normalizeString(query.notif_type || query.notifType || "Delta") || "Delta";

    const { table: resolvedTable, rows } = await readRatesWithFallback({
      hotelId,
      start,
      end,
    });
    ...
    const chunks = chunkRates(normalizedRows);
    const nowIso = new Date().toISOString();
    const echoToken = `rates-${hotelId}-${nowIso.replace(/[^0-9]/g, "").slice(0, 14)}`;

    const lines = [];
    lines.push('<?xml version="1.0" encoding="UTF-8"?>');
    lines.push(
      `<OTA_HotelRateAmountNotifRQ xmlns="http://www.opentravel.org/OTA/2003/05" EchoToken="${escapeXml(
        echoToken,
      )}" TimeStamp="${escapeXml(nowIso)}" Version="3.0" NotifType="${escapeXml(
        notifType,
    ...
    lines.push(`  <RateAmountMessages HotelCode="${escapeXml(hotelId)}">`);
    lines.push(
    ...
    lines.push(
      `    <!-- emitted_messages: ${chunks.length} (only includes dates with is_available=true and nightly_rate present) -->`,
    );

    for (const chunk of chunks) {
      lines.push("    <RateAmountMessage>");
      lines.push(
      ...
      lines.push("      <Rates>");
      lines.push("        <Rate>");
      lines.push("          <BaseByGuestAmts>");
      lines.push(
        `            <BaseByGuestAmt AmountBeforeTax="${chunk.nightlyRate.toFixed(
          2,
        )}" AmountAfterTax="${chunk.nightlyRate.toFixed(2)}" CurrencyCode="${escapeXml(
          chunk.currency,
    ...
    lines.push("  </RateAmountMessages>");
    lines.push("</OTA_HotelRateAmountNotifRQ>");
    lines.push("");
